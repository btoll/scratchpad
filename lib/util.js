...
add = (function () {
    var pushNotefile = function (notefiles, toAdd, fn) {
        if (notefiles.indexOf(toAdd) === -1) {
            notefiles.push(toAdd);
        ...
        getConfigFile(function (err, json, config) {
            ...
            if (notefile) {
                ...
                for (i = 0, len = notefile.length; i < len; i++) {
                    pushNotefile(notefiles, notefile[i]);
                }
                ...
                inquirer.prompt([{
                    type: 'input',
                    name: 'notefile',
                    message: 'Name of notefile:',
                    default: 'foo.txt'
                }], function (answers) {
                    pushNotefile(notefiles, answers.notefile, function () {
                        writeFile(config, JSON.stringify(json, null, 4), function (err) {
                            if (err) {
                                throw err;
                            }
