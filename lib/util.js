...
var exports = module.exports = {},
    ...
    // curl = new Curl(),
    add, remove, decryptFile, encryptFile, generateNewlines,
    getConfigFile, locateConfigFile, writeConfigFile, writeNotefile;
...
add = (function () {
    ...
    return function (newEntry, field) {
        getConfigFile().then(function (res) {
...
exports.addNoteserver = function (noteserver) {
    add(noteserver, 'noteservers');
};

exports.catNotefile = (function () {
    ...
    return function (notefile) {
        if (!notefile) {
            getConfigFile().then(function (res) {
                var json = JSON.parse(res.data),
                    choices = json.notefiles.reduce(function (acc, curr) {
                        acc.push({
                            name: curr,
                            value: curr
                        });
                ...
                inquirer.prompt([{
                    ...
                    type: 'password',
                    name: 'password',
                    message: 'GPG private key: (will not appear as plaintext)',
                    when: function (answers) {
                        return answers.encrypted;
                    }
                }], function (answers) {
                    read(answers.notefile).then(function (data) {
                        if (answers.password) {
                            // TODO
                            decryptFile(answers.notefile, function (err, data) {
                                if (err) {
                                    throw err;
                                }
...
getConfigFile = exports.getConfigFile = function () {
    return new Promise(function (resolve, reject) {
...
exports.init = function () {
    inquirer.prompt([{
        ...
        type: 'list',
        name: 'useGPG',
        message: 'Use encryption? (will use GPG)',
        default: false,
        choices: [
            {name: 'Yes', value: true},
            {name: 'No', value: false}
    ...
    */
    ...
    return function (note, notefile) {
        if (!notefile) {
            getConfigFile().then(function (res) {
                var json = JSON.parse(res.data),
                    fileChoices = json.notefiles.reduce(reduce, []),
                    serverChoices = json.noteservers.reduce(reduce, []);
                ...
                inquirer.prompt([{
                    ...
                    if (answers.encrypted) {
                        encryptFile(note, json.defaults.cipher, function (enciphered) {
                            if (enciphered) {
                                note = enciphered + generateNewlines(json);
                            }
...
remove = (function () {
    ...
    return function (notefile, field) {
        getConfigFile().then(function (res) {
            ...
            if (!notefile) {
                choices = notefiles.reduce(function (acc, curr) {
                    acc.push({
                        name: curr,
                        value: curr
                    });
