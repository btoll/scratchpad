    ...
    Notefile = {
        ...
        makeRequest: (() => {
            ...
            function doWriteFile(notefile, note) {
                writeNotefile(notefile, note).then(() => {
                ...
                if (!notefile) {
                    Notefile.getConfigFile().then((res) => {
                        ...
                        inquirer.prompt([{
                            ...
                            if (answers.encrypted) {
                                // TODO
                                encryptFile(note, json.defaults.cipher, (enciphered) => {
                                    if (enciphered) {
                                        note = enciphered + Notefile.generateNewlines(json);
                                    }

                                    if (local) {
                                        doWriteFile(notefile, note);

                                        // Add the new notefile to the config file.
        ...
        //                            if (enciphered) {
        //                                data.note = enciphered + Notefile.generateNewlines(json);
        //                            }
        //
        //                            send(data);
        //                        });
