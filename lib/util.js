...
var exports = module.exports = {},
    ...
    curl = new Curl(),
    add, remove, decrypt, encrypt, generateNewlines, ciphers,
...
exports.catNotefile = (function () {
    var read = function (notefile, callback) {
        ...
        if (!notefile) {
            getConfigFile(function (err, json) {
                ...
                inquirer.prompt([{
                    ...
                    read(answers.notefile, function (err, data) {
                        var cipher;

                        if ((cipher = answers.cipher)) {
                            decrypt(cipher, data);
                        } else {
                            console.log(data);
                        }
            ...
            read(notefile, function (err, data) {
                console.log(data);
            });
        }
    };
...
decrypt = function (cipher, ciphertext) {
    inquirer.prompt([{
        type: 'password',
        name: 'password',
        message: 'Encryption password:'
    }], function (answers) {
//        var decipher = crypto.createDecipher(cipher, answers.password),
//            dec = decipher.update(ciphertext, 'utf8', 'hex');
